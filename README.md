...
`aqe` is a quality-engineering service that turns a specification into a test plan, runs each step through a browser, desktop, sandboxed CLI, or coding agent adapter, and returns one JSON report.
...
graph TB
    ...
    subgraph Execution["Execution Subsystems"]
        ...
        Sandbox["Docker Sandbox<br/>CLI execution"]
...
3. **Capability Check**: System verifies required drivers (browser, desktop, sandbox, coding agent) are available
4. **Execution**: Steps are routed to appropriate subsystem:
   - GUI steps → Playwright (browser) or PyAutoGUI (desktop)
   - CLI steps → Docker sandbox with synthesized Python scripts
...
- **Process**: Probes host for available capabilities (browser, desktop, sandbox, coding agent, LLM)
...
- **Process**: Executes CLI step through CLISubsystem in Docker sandbox
- **Script Synthesis**: Planner generates Python script for the command
- **Evidence Collection**: Captures stdout, stderr, and execution summary
- **Success**: Transitions to `validate` with ActionResult
- **Error**: Transitions to `finish` with `sandbox_start_failed` or other error codes
...
| CLI | Docker Sandbox | Command-line tool testing |
...
`SPEC_PATH` is the specification file. `aqe run` starts the sample registration app on `SUT_PORT`, plans with the chat model in `.env`, drives browser steps with headless Chromium, desktop steps with PyAutoGUI, checks CLI steps inside the Docker sandbox, and executes coding steps through the Pi agent. The command prints the path to `runs/<id>/report.json`. Exit status is `0` when `verdict` is `pass`.

If Chromium, an X11 display for a desktop step, Docker, the sandbox image, or the Pi agent is missing, the run is rejected before any step executes. A desktop step still requires an X11 display.
...
- `http://127.0.0.1:8000/` submits a specification and lists runs. Chips show whether browser, desktop, sandbox, and coding agent are available on this host.
...
- `GET /v1/capabilities` reports `browser`, `desktop`, `sandbox`, and `coding`.
### HTTP
```bash
...
A pass has `verdict == "pass"`. `specification` is the testing request that was submitted. `reason_code` explains a non-pass (`assertion_failed`, `sandbox_start_failed`, `browser_launch_failed`, `desktop_input_failed`, `driver_timeout`, `engine_error`, `not_a_test_plan`, `missing_capability`, `coding_agent_failed`, `coding_agent_not_available`, or `canceled`). Each step has `assertion_passed` set to `true`, `false`, or `null`.
## Tests
```bash
uv run pytest
