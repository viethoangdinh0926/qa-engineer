...
_EXIT_TERMS = re.compile(
    r"\b(exit codes?|exit status|status codes?|return codes?)\b|exits with",
    re.IGNORECASE,
)


def mentions_exit_code(question: str) -> bool:
    """The exit code belongs in the check only when the assertion talks about it."""
    return _EXIT_TERMS.search(question) is not None
...
_JUDGE_SYSTEM = (
    ...
    "The exit code is included only when the verification is about the exit code, exit status, or return code. "
    ...
    "A non-zero exit code does not fail a check about output or about an expected error. "
...
def present_for_judge(page_source: str, question: str = "") -> str:
    ...
    marker = "\n\nstderr:\n"
    page_source, exit_code = _split_exit_code(page_source)
    ...
    if page_source.startswith("stdout:") and marker in page_source:
        ...
        parts: list[str] = []
        if mentions_exit_code(question) and exit_code is not None:
